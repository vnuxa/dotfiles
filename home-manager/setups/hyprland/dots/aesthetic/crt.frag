...
#define CHROMATIC_ABERRATION_STRENGTH 0.0017
#define DENOISE_INTENSITY 0.0001 //
#define DISTORTION_AMOUNT 0.11 // moderate distortion amount
...
vec4 applyFakeHDR(vec4 color, float brightness, float contrast, float saturation, float bloom) {
    ...
    vec3 crtfactor = vec3(1.0, 1.0, 1.0);
...
vec4 applyChromaticAberration(vec2 uv, vec4 color) {
    ...
    vec4 greenColor = texture2D(tex, uv );

    vec4 result = vec4(redColor.r, greenColor.g, blueColor.b, color.a);
